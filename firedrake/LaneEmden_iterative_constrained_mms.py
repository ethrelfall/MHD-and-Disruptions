# LaneEmden_iterative_constrained_mms.py
...
for i0 in range(0, (res_2d+1)*(res_2d+1)):  # all points
   ...
   r0 = sqrt(x0**2+y0**2)
   u_2d_analytic.dat.data[i0] = conditional(lt(r0, 3.0), 1.0-(r0**2)/9, 2.0*ln(3.0/r0))  # MMS
...
outfile = File("LaneEmden_iterative_constrained_mms.pvd")
...
for i in range(0,20):  # 10 to 20 iters seems sufficient in cases tested
   ...
   rhs.interpolate(rhs-conditional(lt(x_2d**2+y_2d**2, 9.0), (5.0-x_2d**2-y_2d**2)/9.0, 0.0))  # add source
   ...
   for i0 in range(0, (1*res_2d+1)*(1*res_2d+1)):  # all points (really should be only boundary points) - note higher orders don't work like this
      ...
      if ((abs(x0)>5.99) or (abs(y0)>5.99)):  # restrict to bdy points
         ...
         integral = assemble((Greens_func_2d.sub(0)*((-0.5*(1+(tanh((uj_2d.sub(0)+jval)/scale_2d))))*(uj_2d.sub(0)+jval)+conditional(lt(x_2d**2+y_2d**2, 9.0), (5-x_2d**2-y_2d**2)/9.0, 0.0)))*dx)
